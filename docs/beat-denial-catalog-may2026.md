...
60 logs matching `logs/nightbeat/202605{07,08,09,10}*.log` were examined.
...
One session (chemin-de-voix, run `20260510T070140Z`): autonomous raid agent called
`AskUserQuestion` 3× in bypass-permissions mode while working on ticket 0070.
Blocked each time — correct behaviour. Questions go unread until next human session.
Design intent; ticket 0070 carries the design decision that stalled the raid.
