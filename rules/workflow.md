...
**A red gate on `main` is the worst case, because it is a broadcast.** Every parallel session runs the suite, sees the same failure, and the severity floor tells each of them to fix it — so the urgency that makes the repair right is the same thing that makes it collide. Two costs on 2026-09-06: one stale ticket-log stamp was corrected in two sessions at once, whose different but equally defensible values (`15:30Z` and `15:33Z`) conflicted at the merge gate and burned a `/gaze` round; hours later a whole branch, ticket and PR for four more stale stamps was built and closed unmerged, a sibling having landed byte-identical corrections while it was in flight. So do not file the ticket for a shared-metadata repair until the fix is pushed and you know it is yours: a ticket for a repair someone else finished is noise, and dropping it unfiled is cheaper than closing it. None of this touches your own ticket's work, which nobody else is doing.
