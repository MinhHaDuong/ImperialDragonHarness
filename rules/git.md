...
- **Force-push denied → don't rebase a pushed branch; merge as-is.** Rebasing a branch whose PR is pushed rewrites its SHAs, so local diverges from `origin/<branch>` and the ticket-close commit can no longer be pushed — leaving the ticket closed locally and open everywhere else. The rebase's only benefit is currency, which `/verify-gate` reports as a non-blocking nit. So: if the merge state is clean, let it land with a merge commit; only when bases genuinely conflict, integrate with `git merge origin/main` into the branch, never rebase.
...
- **Merge-bounce recovery.** `erg-pr-merge` bounces in sequence and each bounce has its own retry — never blanket-fall back to a plain forge merge. *"close: no ticket found" on a retry*: the first run already closed, archived and pushed (the script is not idempotent past that step) — finish with a direct forge merge once CI is green, and do not hand-close. *"must run from PR branch" after a fast-forward*: HEAD is detached, `git checkout <branch>` and retry.
- **Manual chore-close, only when a PR merged without `erg-pr-merge`.** First confirm no close commit rode the PR. `erg close` edits the ticket without staging it, so `git add -u tickets/` BEFORE the `git mv` to `closed/` — otherwise the commit carries the rename with the pre-edit blob and silently drops the `Closed:` header (a 100%-rename, 0-insertion commit is the tell).
