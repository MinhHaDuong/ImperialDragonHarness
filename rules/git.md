...
- **Create a merge request** for each ticket to review changes before merging. Include a `**Ticket:** tickets/NNNN-...` line in the PR body so `erg-pr-merge` can auto-close the ticket on merge. Use `Ticket-ref: tickets/NNNN-...` to reference a ticket without closing it, or `Ticket: none` for a PR that closes nothing.
