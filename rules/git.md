...
- **Never read a SHA, a count, or a tip from a bare command whose output a proxy may rewrite.** The `rtk` hook compacts for a reader, not a parser: counts came back zero and `git log` dropped merge commits including the range's tip. Use plumbing that answers in exit codes and single values: `git rev-parse`, `git rev-list --count A..B`, `git merge-base --is-ancestor`. The hook's full reach, the escape hatches, and why redirection is **not** one of them: `workflow.md` § Diagnosis discipline.
- **In a worktree session, drive git through `/usr/bin/git -C <worktree>`.** The same `rtk` hook rewrites bare `git X` into `rtk git X`, and the isolation guard, which can only verify a plain command line, then fails closed on *every* git call — piping does not help, since it is the command rewrite it objects to, not the output framing. The absolute path dodges rtk (only the bare token is rewritten) and `-C` is the confinement proof the guard wants. It also refuses compound commands for naming a repo-ish path in a variable, or wrapping `gh --jq` in a loop: put multi-step logic in a scratchpad script and run that.
