...
def _wait_for_unix(path: Path, timeout: float = 5.0) -> None:
    deadline = time.time() + timeout
    while time.time() < deadline:
        if path.exists():
            return
        time.sleep(0.05)
    raise AssertionError(f"relay socket {path} never appeared")
...
def test_relay_unix_to_tcp_round_trip(tmp_path):
    ...
    try:
        ...
        client.sendall(b"ping-through-the-socket")
        ...
        while True:
            ...
            echoed += chunk
        client.close()
        assert echoed == b"ping-through-the-socket"
    finally:
        relay.terminate()
        srv.close()
    ...
    try:
        ...
        while time.time() < deadline:
            try:
                ...
                time.sleep(0.05)
        assert client is not None, "relay TCP listener never came up"
        client.settimeout(5)
        client.sendall(b"pong-through-the-tcp")
        client.shutdown(socket.SHUT_WR)
        echoed = b""
        while True:
            chunk = client.recv(4096)
            ...
            echoed += chunk
        client.close()
        assert echoed == b"pong-through-the-tcp"
    finally:
        relay.terminate()
        usrv.close()
