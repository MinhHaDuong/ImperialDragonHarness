...
- **Field placement is per item type** on the `inject` path: posting a field the target type does not own is a hard 400 from the API. `entry_to_zotero_item()` routes each value through `ZOTERO_SLOT_FIELD` (a reduced snapshot of `api.zotero.org/schema`) — so `publisher` becomes `institution` on a report, `university` on a thesis, and `journal` becomes `bookTitle`, `proceedingsTitle`, `series` or `seriesTitle` depending on the type. Anything with no home lands in Extra under its CSL name; nothing is dropped.
