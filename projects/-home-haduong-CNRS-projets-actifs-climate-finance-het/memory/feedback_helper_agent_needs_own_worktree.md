...
**Merged from `reference_merge_from_foreign_worktree` (2026-09-25):** To merge an agent's PR: in your own worktree git switch --ignore-other-worktrees <branch> && git merge --ff-only origin/<branch>, run erg-pr-merge -C <own worktree> <PR>, then git switch --detach origin/main (staying on the branch leaves the agent's tree looking dirty and blocks worktree-gc).
