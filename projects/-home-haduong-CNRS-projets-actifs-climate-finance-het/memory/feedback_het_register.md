---
name: feedback_het_register
description: "Œconomia manuscript prose must read as History of Economic Thought academic register, not American motivational/business-book cadence"
metadata: 
  node_type: memory
  type: feedback
  originSessionId: 130172cd-69de-440c-879e-e35c7f4a91f5
---

When drafting prose for the Œconomia manuscript (`content/manuscript.qmd`),
avoid the punchy American self-help / business-book register. The author flagged
these specific tics in a §3.1 draft (2026-06-18): "cash out" / "the abstraction
is cashed out"; "Call X and you mean…" (conversational second person); "The
sharper lesson sits in…"; "This is what X is…"; "where the accounting happens";
hammer-short declaratives used as rhetorical beats.

**Why:** the reviewers and venue are HET/STS academic; this is intellectual
history, not a policy brief or a TED talk. Punchy cadence reads as machine-written
or pop-business and undercuts scholarly credibility.

**How to apply:** measured third-person academic prose. "To describe X as Y is to
say…", "The discrepancy records a normative decision…", "is less a category of
money than a procedure for counting it". Keep concreteness and vivid coinages
(e.g. "electrons have no colour") but frame them as idiom/evidence, not slogans.
Complements the [[feedback_ghost_mode]] Economist-style rule and the project
`writing.md` Ghost-mode guidance — this is the HET-specific layer.

**Concrete anchor artifact (added 2026-07-08):** `docs/style-anchor-v205.md`
holds the author's own pre-AI English voice — validated passages from his
1997–2011 first-authored articles (ticket 0173). Draft manuscript prose against
it *from the first draft*, not as a fix-up. The author's actual voice: short
declarative sentences that LAND a point (not recap), one concrete analogy for an
abstract idea (leaky CO2 storage = a bank loan), one measured hedge at a time,
occasional question-close and light irony, grammatically correct British English.

Two failure poles, both rejected, both fixed by rewriting against the anchor:
(1) American punchy/business-book beats (the 2026-06-18 §3.1 case above);
(2) ornate AI-academic long subordinate chains (2026-07-08, PR #894 loss & damage
draft — author: "I don't recognise my voice"). The author's short declaratives
are the cure for pole (2), but keep them plain and concrete — do not tip into the
slogan-y hammer-beats of pole (1). When a draft is rejected on voice, the
procedure is: read `docs/style-anchor-v205.md`, rewrite, re-run the prose ratchet.

**Merged from `feedback_letter_voice_flags` (2026-09-25):** Recurring flags: A-not-B antithesis cadence (keep at most ~3 that carry argument), stacked -ly adverbs, engineering metaphors ('load-bearing'), 'cashes out'. Letters in first person singular.
