...
- **Vérifier sur pièce, jamais sur la foi d'un intermédiaire.** Le corpus redécouvre la même leçon sous cinq formes : un candidat classé secondaire dans un ticket, une occurrence de grep sans son contexte, un `.blg` non lu après une purge de bibliographie, une typographie rétro-portée sans relire la langue du texte, un folio interpolé depuis une extraction. La pièce dit ce qui est ; le registre dit seulement ce qu'il a.
...
- **La sortie « clean » d'un outil n'est pas une preuve, et son silence non plus.** Un merge dit « clean » sur une base périmée, un scan qui ne peut pas regarder rend le même vide qu'un scan qui n'a rien trouvé, un `git checkout <ref> -- fichier` détruit l'index sans rien afficher. Tout garde dont l'« tout va bien » est indiscernable de « je n'ai pas regardé » doit être éprouvé sur un cas positif connu.
