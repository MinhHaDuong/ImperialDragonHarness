...
**Why:** the two spawn mechanisms differ in their tool schema, not just convention — `Agent` genuinely has no `effort` field, `Workflow.agent()` genuinely does. Treating them as the same risks leaving `Workflow` fan-outs stuck at session effort when a cheap mechanical stage should run at `low` or a hard verify/judge stage at `high`+.
