...
- `$PWD/.env` — project-level, UNTRUSTED (an agent/clone/project write can place it): **strict-parsed** `KEY=VALUE`, never sourced. Values assigned LITERALLY (no eval/`$()`/backticks), one surrounding quote-pair stripped, non-identifier keys skipped. Any guard-namespaced key (substring `GUARD_`, covering `GUARD_*` and the `_GUARD_*` override pair) is REFUSED, so it cannot forge a per-process guard nonce or the worktree-path override. **Ticket 0345 widened this:** the strict-parse loop now ALSO refuses every shell/process/interpreter-critical export NAME (see the shared-predicate bullet below), because the untrusted file controls both name and value — an un-denied critical name is a direct RCE/clobber vector (`GCONV_PATH`, `LD_PRELOAD`, `BASH_ENV`, `PATH`, `PYTHONPATH`, …) in every subprocess. Hit → `bash-env: refusing protected name from project .env: <name>`, line skipped.
