...
**Protected export-name refusal (ticket 0343):** the `KEYS=` explicit-selection forms (`provider:VAR`, `provider:SRC=DST`) let the untrusted `.env` pick the EXPORT NAME. Besides the `*GUARD_*` / `_be_*` refusal, the export name (DST, or SRC in the no-rename form) is refused when it names a shell/process-critical variable — exact: PATH, BASH_ENV, ENV, SHELLOPTS, BASHOPTS, IFS, PS1..PS4, PROMPT_COMMAND, CDPATH, GLOBIGNORE, LD_PRELOAD, LD_LIBRARY_PATH, LD_AUDIT; prefixes: `BASH_FUNC_*`, `DYLD_*`. Hit → `bash-env: refusing KEYS export to protected name: <name>`, entry skipped (default-deny). Closes the hole where `KEYS=p:VAR=PATH` (or `=BASH_ENV`/`=LD_PRELOAD`/`=IFS`) could overwrite a critical variable in every subprocess.
