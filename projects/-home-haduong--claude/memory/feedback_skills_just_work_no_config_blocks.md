...
The directive extends to missing *arguments* (2026-06-10): a skill that refuses a missing PR number instead of resolving it from the forge (branch → open merge request) is the same pre-authoring chore in interactive form — and a refusal message that invents a concrete example ID anchors the user into re-invoking on the wrong target (/review-pr suggested "e.g. 378" when the conversation was about #380; the user pasted it and a five-agent panel ran on an already-merged PR). Resolve from authoritative state, present real candidates when ambiguous, never fabricate plausible-looking IDs in error messages. Automation gates (gaze, verify-gate, verify-adherence) keep their refuse-on-missing-arg contract — they are invoked programmatically and return error codes, not suggestions.
