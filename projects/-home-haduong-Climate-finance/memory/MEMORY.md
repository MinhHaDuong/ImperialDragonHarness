...
- The corpus pipeline degrades silently at several joints (GROBID down, stale Feather caches, undeclared optional deps): a headline number that moves between passes is the alarm — verify the producing stage before citing any count. The Flag-6-without-torch joint is now a hard error (0314, PR #1127); `diptest` in analyze_bimodality is the same class still open (0330).
...
- Environment values shadow: ambient shell exports beat `--env-file`, and secrets belong in the harness env, never the project `.env`. But an in-code `load_dotenv()` beats both — `pipeline_loaders` reads the *worktree's* `.env`, so an exported `CLIMATE_FINANCE_DATA` does not reliably redirect a script to the real corpus (0347).
...
- Generated markup is only as good as its rendering: a source-level test cannot see a rendering bug, and an emitter writing raw LaTeX/Markdown owns escaping the renderer would otherwise do (0325 published an inverted filter recipe from a byte-correct source file). The escaper is chosen by the *sink's decoder*, not the markup language — Plotly decodes eight entities and no `quot`, so `html.escape(quote=True)` is right for a browser-parsed attribute and wrong for hover text (0341).
- Test the artifact a script writes, never the record it builds: a `columns=` allowlist drops a computed field silently, and 41 passing tests missed it for five days because every one asserted on the in-memory dict (0288/0347). Its blind spot: a value silently coerced to blank in a *nullable* column validates cleanly, so strictness belongs at the write step (0354).
- A guard that pattern-matches source text is always one spelling behind: when the defect has a data signature, assert the property in the code path and keep the static check only for routes that erase the signature (0402 — regex, then AST, then runtime, each broken by review in turn).
- Deleting a generated artifact is never a local change — guards and allowlists on other branches key on its filename and fire only when the branches meet, so budget a post-merge round per merge of main (0354: three collisions, two merges).
